...
def crear_app():
    ...
    def login():
        if request.method == 'POST':
            ...
            user = app.db.InicioSesion.find_one({'username': username})
    ...
    def register():
        if request.method == ['POST']:
            try:
                ...
                app.db.InicioSesion.insert_one({
                    'username': username,
                    'email': email,
    ...
    def add_data():
        try:
            data = request.json
            if data:
                app.db.InicioSesion.insert_one(data)
    ...
    def listar_discusiones():
        ...
        per_page = 10
        total = app.db.discusiones.count_documents({})
        discusiones = app.db.discusiones.find().skip((page - 1) * per_page).limit(per_page)
        autores = {str(user['_id']): user['username'] for user in app.db.InicioSesion.find()}
    ...
    def nueva_discusion():
        ...
        if request.method == 'POST':
            ...
            autor = app.db.InicioSesion.find_one({'_id': ObjectId(autor_id)})
    ...
    def ver_discusion(id):
        ...
        discusion = app.db.discusiones.find_one({'_id': ObjectId(id)})
        if discusion:
            comentarios = list(app.db.comentarios.find({'discusion_id': ObjectId(id)}))
            autores = {str(user['_id']): user['username'] for user in app.db.InicioSesion.find()}
